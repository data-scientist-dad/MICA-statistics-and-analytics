...
# How could you get the CI when you only have the mean and standard deviations
...
# The DEA thinks a farmer is growing poppies on a 3000 acre property.
...
ci.twomeans(30,10,25,12, sd1 = 11, sd2 = 12, vname= "probably the mean")
...
# Your friends have an average weight loss of 8.6 with a sample standard deviation of 6.5
...
ci.paired(100,8,s.d = 4, conf.level = .99, vname= "score difference") #set the confidence level to .99
...
## that the proportion of males with cancer is significantly different from 0.5 with a p-value = 0.01771.
...
##EXAMPLE ONE:
data("mtcars")
lmfit1 <- lm(mtcars$mpg ~ mtcars$cyl)
lmfit1
