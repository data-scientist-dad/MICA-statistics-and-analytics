##Week Six : Difference and Hypothesis Testing
...
virginica <- iris %>%
  filter(Species == "virginica") #note that I screwed this up in the posted script

dim(virginica)
glimpse(virginica)

ggplot(virginica)  + 
  geom_point(aes(x = Sepal.Length, y = Sepal.Width, color = "gold")) + 
...
# IN THIS PARTICULAR KIND OF TEST if you want to find the t-statistic for this test by hand:
...
# Wait...what is CDF?  
# Simply stated it calculates the cumulative probability for a given x-value.
...
#it gives the area under the probability density function from minus infinity to x.

# Check out the commented out link at CRAN to use the cdf() command.
# you can check your work (step 7) after getting a p-value the other way.
# however...here is one short hand, informal way.

# A student wants to estimate how much time people spend showering daily
# step (0)

# read in the data
x <- c(3, 7, 11, 2, 7, 14, 4, 5, 6, 2)
...
# Compare two variances of two independent samples
...
t.test(data1$var5, data1$var6, var.equal = FALSE, paired = FALSE)
...
# one group, and compares it to the same thing, but measured in ANOTHER group.
...
## One more chi-square test for goodness of fit test for the road...
## each category must have a frequency of at least five

table(sleep$group)

chisq.test(table(sleep$group))

## ANOTHER NON-PARAMETRIC TEST EXAMPLE
...
## correct hypothesis test for your situation.